...
class Pengguna extends AUTH_Controller {
	...
	public function index() {
		...
		if (($this->session->userdata('id_role') == 1) || ($this->session->userdata('id_role') == 2)) {
			...
			$data['modal_update_pengguna'] = show_my_modal('modals/modal_update_pengguna', 'update-pengguna', $data);
	...
	public function export() {
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, "Telp");
		$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, "Created On");
		$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, "Last Login");
		$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, "Aktif?");
		$rowCount++;
		...
		foreach($data as $value){
		    ...
		    $objPHPExcel->getActiveSheet()->setCellValueExplicit('F'.$rowCount, $value->phone, PHPExcel_Cell_DataType::TYPE_STRING);
		    $objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, $value->created_on); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, $value->last_login); 
		    $objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, $value->active); 
		    $rowCount++; 
		} 
				...
				foreach ($sheetData as $key => $value) {
					if ($key != 1) {
						...
						if ($check != 1) {
							...
							$resultData[$index]['email'] = $value['E'];
							$resultData[$index]['phone'] = $value['F'];
							$resultData[$index]['active'] = $value['G'];
						}
					}
					$index++;
				}
